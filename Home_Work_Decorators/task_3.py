...
Курсы в процессе изучения: {" " if not self.courses_in_progress else
        ", ".join(self.courses_in_progress)}
Завершенные курсы: {" " if not self.finished_courses else
        self.finished_courses}"""
    ...
    def new_function(*args, **kwargs):
        ...
        with open('task_3.log', 'a') as log:
            log.write(f'{old_function.__name__}, '
                      f'arguments: {str([*args, *kwargs.values()])}, result: {result}\n')
        return result
    return new_function
